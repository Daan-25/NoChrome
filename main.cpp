...
struct StyleRule {
    ...
    std::string tag;      // for Tag/TagClass
    std::string cls;      // for Class/TagClass
...
static std::optional<StyleRule> parseSingleSelectorRule(const std::string& selectorRaw,
    ...
    // Reject complex selectors early
...
struct StyledToken {
    ...
    std::string href;
    Style style;
    int breakCount = 1; // Only used when kind == Break
};
...
static int defaultBreakCountForTag(const std::string& tagNameLower) {
    // This is a simple spacing heuristic for v0 layout
...
static std::vector<StyledToken> parseHtmlToStyledTokens(const std::string& html) {
    ...
    // Remove non-rendered blocks from content stream
    ...
    auto processTag = [&](const std::string& raw){
        ...
        // Start tag: compute element style
        Style st = currentStyle();
    ...
    int lineHeight)
{
    ...
    auto tokens = parseHtmlToStyledTokens(page.body);
...
static void drawFilledRect(SDL_Renderer* r, const SDL_Rect& rect, Uint8 R, Uint8 G, Uint8 B, Uint8 A=255) {
...
int main(int argc, char** argv) {
    ...
    if (!loadFontSet(fonts)) {
        std::cerr << "Could not load a font. Add ./fonts/DejaVuSans.ttf\n";
    ...
    // Fixed line height for v0 layout
    int lineHeight = 32;
    ...
    while (running) {
        ...
        drawFilledRect(renderer, contentBg, 18, 18, 20);
        ...
        for (int i = startIndex; i < endIndex; ++i) {
            ...
            for (const auto& sp : line.spans) {
                if (sp.texture) {
                    ...
                    if (!sp.href.empty()) {
                        SDL_SetRenderDrawColor(renderer, sp.style.color.r, sp.style.color.g, sp.style.color.b, 255);
